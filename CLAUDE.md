...
- Android Auto integration (`android/app/src/auto/kotlin/com/code418/postbox_game/car/` — `PostboxCarAppService`, `HomeCarScreen` with stats pane + quick-claim, `LeaderboardCarScreen`, `StatsRepository` reads Firestore directly since the car runtime never loads the Dart engine). Declared as a POI app (`androidx.car.app.category.POI`); `ClaimAction` calls `startScoring` directly. **Lives only in the `auto` product flavour, which is distributed OUTSIDE Google Play.** Google Play rejected the car app **twice** under the Auto app-quality guidelines ("no meaningful functionality relevant to driving") — first the full stats+leaderboard build, then even a slimmed quick-claim-only POI build — so the **Play-bound `phone` flavour carries no Android Auto at all** (no `<service>`/`car.application` meta-data/`automotive_app_desc.xml`, and the car deps are `autoImplementation`, not `phoneImplementation`). Three Gradle flavours now share one applicationId: `phone` (Play, no Auto), `auto` (off-Play, full Auto; `flutter build appbundle --flavor auto -t lib/main.dart`), `wear`. The full car code was restored from commit `42971e2` (the slim-down was `3543842`). **Follow-up:** the off-Play `auto` build uses the `AndroidPlayIntegrityProvider` App Check provider, which expects Play distribution — confirm App Check enforcement state / Play Integrity behaviour before relying on it (see the plan's out-of-scope note).
