...
- Android Auto integration (`android/app/src/phone/kotlin/com/code418/postbox_game/car/` — `PostboxCarAppService`, `HomeCarScreen` quick-claim only; `phone` product flavour only). Declared as a POI app (`androidx.car.app.category.POI`). Deliberately a single status row + "Claim nearby postbox" action — **no** points/streak/rank/leaderboard on the driving surface (Google rejected the earlier stats-pane + leaderboard build under "no meaningful functionality relevant to driving"; that game/social content lives in the phone app only). `ClaimAction` calls `startScoring` directly (the car runtime never loads the Dart engine). If Play rejects the car app again, the reliable fallback is to drop the `<service>` + `com.google.android.gms.car.application` meta-data + `automotive_app_desc.xml` from the `phone` manifest, leaving the `car/` code in-tree but un-advertised.
