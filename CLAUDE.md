...
- `functions/src/test/test.index.ts` uses `firebase-functions-test`. 262 TypeScript tests passing (pure unit tests + auth/validation integration tests that gracefully skip when no emulator is running). Includes tests for `updateFcmTokens`, `diffFriends`, `shouldNotifyFirstClaim`, `shouldNotifyOvertake`, `buildOsmChange`, `parsePhotos`, `pointsForMonarch`, `maxDailyFromClaims`, and `submitReport`/`reviewReport` auth & validation.
...
- **Rate limiting / abuse**: Consider rate limits on callables (incl. a per-uid daily cap on `submitReport`) and abuse detection for claims (e.g. same device, location spoofing).
