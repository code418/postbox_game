/**
 * Formats an arbitrary instant as its "YYYY-MM-DD" calendar day in
 * Europe/London (BST/GMT handled automatically). Added in v1.5 so the offline
 * flush path can derive the London day a capture belongs to from its verified
 * capture time.
 */
export function getLondonDayOf(date: Date): string {
...
export function getTodayLondon(): string {
  return getLondonDayOf(new Date());
